...
def run_train(model, dataloader, criterion, optimizer, device, args, use_gene_idx=True):
    ...
    for i, dataset in enumerate(dataloader):
        ...
        if args.evaluate_task == 'niche_classification' and args.method == 'ours-MLP':
        ...
        if args.evaluate_task in ['macrophage_classification', 'he_annotation_classification', 'niche_classification']:
...
def run_test(model, dataloader, device, args, use_gene_idx=True):
    ...
    for dataset in dataloader:
        ...
        if args.evaluate_task == 'gene_regression':
            ...
            metric = calculate_score(labels_eval, outputs_eval, 'Accuracy_TopK', test_idx, class_num=args.cell_type_num)
        elif args.evaluate_task == 'cell_lineage_classification':
            metric = calculate_score(labels_eval, outputs_eval, 'Accuracy', test_idx)
        elif args.evaluate_task in ['he_annotation_classification', 'niche_classification']:
            cell_mask = data.cell_mask.to(device)
            if cell_mask.sum() == 0:
                continue
            outputs_eval = outputs_eval[:, 1]
            metric = calculate_score(labels_eval, outputs_eval, 'AUC', cell_mask)
        ...
        test_metric = test_metric + metric / n
        ...
        if args.evaluate_task == 'gene_regression':
            for t in range(len(args.hvg_gene_tops)):
                ...
                for m in range(num_metrics):
                    running_metrics[num_metrics*t+m] = running_metrics[num_metrics*t+m] + metrics[m] / n
        else:
            if args.evaluate_task in ['macrophage_classification', 'he_annotation_classification', 'niche_classification']:
                test_idx = data.cell_mask.to(device)
                if test_idx.sum() == 0:
                    continue
            if args.evaluate_task in ['he_annotation_classification', 'niche_classification']:
                outputs_eval = outputs_eval[:, 1]
            metrics = calculate_metrics(labels_eval, outputs_eval, args.metrics, test_idx, class_num=args.cell_type_num)
            for m in range(num_metrics):
                running_metrics[m] = running_metrics[m] + metrics[m] / n
